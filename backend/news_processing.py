...
def classify_news(title, full_text):
    """Classifies news based on risks."""
    combined_text = f"{title}. {full_text if full_text else 'NA'}"
    classification = classifier(combined_text, risk_labels)
    return classification["labels"][0]# Top result
